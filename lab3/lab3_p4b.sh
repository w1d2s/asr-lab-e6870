...
python ./get_nbest_stat.py all.nbest.orign > nbest_stat
...
for d in 2000 20000 200000 232838 ; do
    ...
    python rescoring.py all.nbest.$d.prob nbest_stat 4.0 > all.nbest.$d.rec

    # convert UH and UM to (%HESITATION)
    python convert_hesitation.py  all.nbest.$d.rec >  all.nbest.$d.rec2
...
if [[ ! -e all.nbest.$d.prob ]] ; then
    ...
    for lmw in 2.0 3.0 4.0 ; do
        python rescoring.py all.nbest.$d.prob nbest_stat $lmw > all.nbest.$d.$lmw.rec

        # convert UH and UM to (%HESITATION)
        python convert_hesitation.py  all.nbest.$d.$lmw.rec >  all.nbest.$d.$lmw.rec2
